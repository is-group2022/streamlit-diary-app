...
# タブ構成の更新（Tab3, Tab4を削除し、Tab5, Tab6を繰り上げ）
...
with tab1:
    ...
    with st.expander("📖 はじめての方へ：新規データ登録の使い方（クリックで開閉）", expanded=False):
        ...
        * ※エリアと店名は、後の画像検索や管理の基準となるため正確に入力してください。
        ...
        * 画像を選択すると、自動的に「エリア/店名」フォルダへリネームして保存されます。
                ...
                for idx, (area_name, shops) in enumerate(areas.items()):
                    with area_cols[idx]:
                        ...
                        for shop in sorted(shops):
                            # チェックボックスの表示のみ維持（移動ロジックは削除）
                            st.text(f"🏢 {shop}")

# =========================================================
# --- Tab 3: 📚 ③ 使用可能日記文 (旧Tab 5から繰り上げ) ---
# =========================================================
with tab3:
